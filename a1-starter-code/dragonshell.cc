    ...
    for (unsigned int i = 0;i < tokenizer.size(); i++){                           // loop through the separated commands
      ...
      for (unsigned int i = 0; i < tokenizer_output_addon.size(); i++){                           // loop linearly through the " " tokenized individual commands to find |, &, or >
        ...
        if (tokenizer_output_addon.at(i) == "|"){
          piping_bool = true;                                                                     // piping is true if | is found.. implementation not done
...
void a2path_implementation(std:: vector<std::string> s){
  ...
  std:: string colon = ":";
  if ((s.size() > 1)){
    std:: vector<std:: string> new_s = tokenize(s.at(1),colon.c_str());
    if ((new_s.at(0) == "$PATH")){
      ...
      if (append.size() > 1){ 
        PATH = PATH + colon + append.at(1);
      }                                             // add a colon to the global string and append the user tokenized string after :
    }
  }
} 
// implement exit  
